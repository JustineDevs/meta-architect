...
- Do not commit runtime `.meta-architect` state such as logs, state snapshots, or local caches.
...
- `.meta-architect/skills/` — internal skill logic
...
When changing any skill in `skills/` or `.meta-architect/skills/`:
...
If uncertain, ask before committing new `.meta-architect` paths.
...
- no forbidden `.meta-architect` runtime files are staged
...
- [ ] I did not commit runtime `.meta-architect` state.
