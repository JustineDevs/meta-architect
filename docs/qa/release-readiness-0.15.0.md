...
`v0.15.0` is treated as production only when:
...
- release docs, package metadata, and workflows all agree on `0.15.0`
...
- version/tag alignment `0.15.0` / `v0.15.0`: PASS
...
- tests pass: PASS
- live Jev-backed regression: PASS; `docs/qa/live-regression-0.15.0.json` records setup, doctor, `$maestro`, source mutation, verification, receipt, and cleanup
...
- issue proof gates for `v0.15.0`: PASS; `release-issue-gates-0.15.0.json` marks issues `#13-#29` passed with implementation, verification, and production evidence, so release verification can enforce the gate artifact instead of blocking on pending issue states
...
Every open issue assigned to `v0.15.0` must remain in this file until it is production-passed. A passed issue requires:
...
```bash
ma setup
ma idea "Harden Meta-Architect v0.15.0 semantic core with Obsidian vault context, Ralph execution proof, context economy, and package-gated release evidence"
ma run '$arch'
ma run '$sage'
ma run '$flow'
ma run '$vet'
