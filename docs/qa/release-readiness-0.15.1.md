# Release Readiness 0.15.1

## Production bar

`v0.15.1` is treated as production only when:
...
- release docs, package metadata, and workflows all agree on `0.15.1`
...
- version/tag alignment `0.15.1` / `v0.15.1`: PASS
...
- tests pass: PASS
- live Jev-backed regression: PASS; `docs/qa/live-regression-0.15.1.json` records setup, doctor, `$maestro`, source mutation, verification, receipt, and cleanup
...
- issue proof gates for `v0.15.1`: PASS; `release-issue-gates-0.15.1.json` marks issues `#13-#29` passed with implementation, verification, and production evidence, so release verification can enforce the gate artifact instead of blocking on pending issue states

## Automated checks run

```bash
npm run release:check
npm run live:regression -- --output docs/qa/live-regression-0.15.1.json
```
...
Every open issue assigned to `v0.15.1` must remain in this file until it is production-passed. A passed issue requires:
...
```bash
ma setup
ma idea "Harden Meta-Architect v0.15.1 semantic core with Obsidian vault context, Ralph execution proof, context economy, and package-gated release evidence"
ma run '$arch'
ma run '$sage'
ma run '$flow'
ma run '$vet'
