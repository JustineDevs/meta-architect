...
interface ChatSidebarProps {
  ...
  onGenerationComplete?: (code: string, summary?: string, metadata?: AssistantMetadata) => void;
  onErrorMessage?: (message: string, errorType: "edit_failed" | "api" | "validation") => void;
...
export const ChatSidebar = forwardRef<ChatSidebarRef, ChatSidebarProps>(function ChatSidebar(
  ...
  const handleGeneration = async (options?: { silent?: boolean; attachedImages?: string[] }) => {
      ...
      {isCollapsed ? (
        ...
        <div className="flex-1 flex flex-col min-h-0 ml-12 mr-8 mb-8 rounded-xl bg-muted/20 border border-border/30 shadow-sm">
          ...
          <div className="flex items-start justify-between px-4 pt-4 pb-2">
            <h2 className="text-sm font-medium text-muted-foreground">Assistant Chat</h2>
          ...
          <ChatInput
            ...
            isLoading={isLoading}
            onSubmit={(attachedImages) => handleGeneration({ attachedImages })}
            Component={Component}
            fps={fps}
