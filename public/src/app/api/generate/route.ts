...
const FollowUpResponseSchema = z.object({
  ...
  edits: z
    .array(
      z.object({
        ...
        old_string: z.string().describe("The exact string to find (must match exactly)"),
  ...
  code: z
    .string()
    .optional()
    .describe("Required when type is 'full': the complete replacement code starting with imports"),
});
...
The old_string was either not found or matched multiple locations. You MUST include more surrounding context to make the match unique.`
        ...
        const isEditFailure =
          errorCorrection.error.includes("Edit") && errorCorrection.error.includes("failed");
